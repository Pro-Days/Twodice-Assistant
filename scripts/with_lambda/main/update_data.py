...
# logging.basicConfig(filename="thread_log.log", level=logging.INFO, format="%(message)s")
...
#             for i in data:
#                 rankdata += (
#                     f",{misc.get_uuid(data[i]["name"])}-{data[i]["level"]}-{misc.convert_job(data[i]["job"])}"
#                 )
#             rankdata += "\n"
