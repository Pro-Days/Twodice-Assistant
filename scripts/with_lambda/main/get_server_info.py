...
def get_server_info(period):
    ...
    if (df["players"].iloc[-1] == df["players"].iloc[-2]) and (df["votes"].iloc[-1] == df["votes"].iloc[-2]):
    ...
    x_new = np.linspace(x.min(), x.max(), (len(df_uniqueV["timestamp"]) - 1) * (smooth_coeff - 1) + 1)
...
def get_player():
    try:
        ...
        try:
            ...
            player = int(server_info["players"]["online"]) if server_info["online"] else 0
